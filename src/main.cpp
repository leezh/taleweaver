...
int main(int argc, const char *argv[]) {
    ...
    auto cameraPos = glm::vec3(0.f, 2.f, 5.f);
    auto view = glm::translate(glm::mat4(1.0), glm::vec3(0.f, 2.f, 0.f));
    while (running) {
        SDL_Event event;
        ...
        if (state[SDL_GetScancodeFromKey(SDLK_a)]) turn += 40.f;
        if (state[SDL_GetScancodeFromKey(SDLK_d)]) turn -= 40.f;
        ...
        view = glm::translate(view, input * 3.f * delta);
